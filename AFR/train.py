...
def train(args, trial=None):
    ...
    # 设置每100步打印一次日志
    print_step = 100  # 每100步显示一次
    losses = []
    args.save_score = -float("inf")  # 确保任何结果都会保存
    for step in tqdm(range(1, args.max_step + 1), total=args.max_step, dynamic_ncols=True):
        ...
        # 4. validation
        # 更新训练循环中的日志输出
        if step % print_step == 0:  # 每100步进行一次验证和输出日志
            early_stop_count += 1
            ...
            # Print log with immediate flush (without using flush in logger.info)
            loss_str = "".join(f" {k}:{v:2.4} |" for k, v in loss.items())
            eval_str = "".join(f" {k}:{v:2.4} " for k, v in eval_results.items() if
                               k not in ["frame_level_report", "video_level_report"])
            logger.info("Step %d | NCrop:%s|%s %s", step, eval_str, loss_str, improve_str)
            sys.stdout.flush()  # 强制刷新

    if best_result:
        save_data = {**vars(args),
                     **{k: v for k, v in best_result.items() if k not in ["frame_level_report", "video_level_report"]}}
...
def train_one_step(
    ...
    if args.amp:
        ...
        if not skip_lr_sched:
            scheduler.step()  # This updates the learning rate scheduler if AMP scaling is applied
    else:
        loss.backward()
        optimizer.step()
        scheduler.step()
...
def objective(trial, args, freeze_args):
    params = {
         ...
         "gamma": trial.suggest_float("gamma", 1, 3.0, step=0.5),
         "threshold": trial.suggest_float("threshold", 0.4, 0.5, step=0.05),
         "lr_scheduler": trial.suggest_categorical("lr_scheduler", ["OneCycleLR", "constant"]),
         "max_seq_len": trial.suggest_categorical("max_seq_len", [256, 384]),
         "arch": trial.suggest_categorical("arch", ["1_2_5", "2_1_5", "2_2_4"]),
         "n_mha_win_size": trial.suggest_categorical("n_mha_win_size", [4, 8]),
         "n_embd": trial.suggest_categorical("n_embd", [256, 512, 1024]),
         "n_head": trial.suggest_categorical("n_head", [4, 8]),
         "se_ratio": trial.suggest_categorical("se_ratio", [4, 8]),
